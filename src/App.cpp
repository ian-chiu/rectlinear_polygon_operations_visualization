...
struct App::AppConsole
{
    ...
    void    Draw(const char* title, bool* p_open)
    {
        ...
        else
        {
            ImGui::TextWrapped(
                "commands:\n"
                "\tsplit - change to split mode\n"
                "\tnon-split - change to non-split version\n"
            );
        }
    ...
    void    ExecCommand(const char* command_line)
    {
        ...
        if (!connect_app->isAllDone)
        {
            if (Stricmp(command_line, "skip") == 0) 
            {
                ...
                AddLog("%s", (connect_app->curr_oper + " processing...").c_str());
            }
            else
            {
                ...
                if (connect_app->step_cnt >= 0)
                    AddLog("execute %d steps\n", connect_app->step_cnt);
            }
        }
        else
        {
...
// {
//     std::string oper{operations_queue.front()};
